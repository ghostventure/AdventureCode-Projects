...
export default function ExcelBolt() {
  ...
  const [usage, setUsage] = useState({ exports: 18, connectors: 3, rowsThisMonth: 14200 });
  const [connected, setConnected] = useState(["quickbooks", "stripe", "shopify"]);
  ...
  const [favorites, setFavorites] = useState(["pnl", "sales"]);
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (meta && e.key === "k") { e.preventDefault(); setCmdPalette(p => !p); setCmdQuery(""); }
      if (e.key === "?" && !e.target.closest("input,textarea,select")) { e.preventDefault(); setShowShortcuts(p => !p); }
      if (meta && e.key === "d") { e.preventDefault(); setDarkMode(p => !p); }
      if (e.key === "Escape") { setCmdPalette(false); setShowShortcuts(false); }
      // Number keys for tab switching
      if (meta && e.key >= "1" && e.key <= "7" && !e.target.closest("input,textarea")) {
        e.preventDefault();
  ...
  const cmdActions = [
    ...
    { id:"admin", label:"Admin Console", desc:"Support tickets and system health", keys:"", icon:"🛡️", action:()=>{ setCmdPalette(false); switchTab("admin"); } },
    ...
    { id:"logout", label:"Sign Out", desc:"End session", keys:"", icon:"🚪", action:()=>{ setCmdPalette(false); setIsLoggedIn(false); setAuthUser(null); } },
  ];
  ...
  const generateRealExcel = (template) => {
    ...
    sheets.forEach((sheetName, si) => {
      xml += `<Worksheet ss:Name="${sheetName.substring(0,31).replace(/[^\w\s]/g,"")}"><Table>`;
      // Column widths
      cols.forEach(() => { xml += '<Column ss:AutoFitWidth="1" ss:Width="120"/>'; });
      // Header row
      xml += '<Row ss:StyleID="header">';
      if (si === 0) { cols.forEach(c => { xml += `<Cell ss:StyleID="header"><Data ss:Type="String">${c}</Data></Cell>`; }); }
      else { cols.forEach((c,ci) => { xml += `<Cell ss:StyleID="header"><Data ss:Type="String">${c} (${sheetName})</Data></Cell>`; }); }
      xml += '</Row>';
      // Data rows
      ...
      allRows.forEach((row, ri) => {
        ...
        xml += `<Row${style}>`;
        row.forEach((cell, ci) => {
          const isNum = typeof cell === "string" && (cell.startsWith("$") || cell.startsWith("+") || cell.startsWith("-") || cell.endsWith("%"));
          const cleanVal = String(cell).replace(/[$,%+]/g, "").replace(/,/g, "");
  ...
  const generateJWT = (email, name) => {
    ...
    const payload = btoa(JSON.stringify({ sub: email, name, iat: Math.floor(Date.now()/1000), exp: Math.floor(Date.now()/1000) + 86400, sid: `sess_${Date.now().toString(36)}` }));
    const sig = btoa(Array.from({length:32},()=>"abcdef0123456789"[Math.floor(Math.random()*16)]).join(""));
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, fontFamily: "'DM Sans','Segoe UI',sans-serif", color: theme.text, transition: "background 0.3s, color 0.3s" }}>
      ...
      {/* ═══ Nav ═══ */}
      <nav style={{ background: theme.nav, padding: "0 28px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: dm?"none":"0 2px 24px rgba(27,94,32,0.3)", borderBottom: dm?`1px solid ${theme.border}`:"none", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 10, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", color: "#A5D6A7" }}><I.Bolt s={20} /></div>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {[
            ...
            { id: "admin", label: "Admin", icon: <I.Terminal s={14}/> },
          ].map((t) => (
      ...
      <div style={{ maxWidth: 1220, margin: "0 auto", padding: "24px 20px 60px" }}>
          ...
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
              {[
                ...
                <div key={i} style={{ background: "#fff", borderRadius: 14, padding: "20px 22px", border: "1px solid #E4EDE2", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            <div style={{ display:"flex",justifyContent:"space-between",marginBottom:18 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:8,background:"#fff",border:"1px solid #DDE8DA",borderRadius:10,padding:"7px 14px",width:240 }}>
                <I.Search s={13}/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search..." style={{border:"none",outline:"none",background:"transparent",fontSize:12,color:"#1a2e1a",width:"100%",fontFamily:"inherit"}}/>
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              {filteredConn.map((c) => {
                ...
                return (
                  <div key={c.id} style={{ background:"#fff", borderRadius:14, padding:20, border:on?"2px solid #43A047":"1px solid #E4EDE2", opacity:soon?0.55:1, position:"relative" }}>
          ...
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:14}}>
              {filteredTpl.map((t) => {
                ...
                return (
                  <div key={t.id} style={{ background:theme.card, borderRadius:14, padding:24, border:`1px solid ${isBatchSel?"#43A047":theme.border}`, display:"flex", flexDirection:"column", position:"relative", opacity:locked?0.85:1, boxShadow:isBatchSel?"0 0 0 2px #43A04744":"none", transition:"all 0.15s" }}>
                    {/* Batch checkbox */}
        ...
        {tab === "exports" && (
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{background:"#fff",borderRadius:14,border:"1px solid #E4EDE2",overflow:"hidden"}}>
              ...
              {[
                {id:1,name:"Q1_2026_PnL.xlsx",template:"Monthly P&L Statement",date:"Mar 22",rows:342,sheets:4,size:"248 KB"},
                {id:2,name:"March_Sales.xlsx",template:"Sales Dashboard",date:"Mar 21",rows:1205,sheets:4,size:"1.1 MB"},
                {id:3,name:"Inventory_Mar.xlsx",template:"Inventory Tracker",date:"Mar 20",rows:89,sheets:3,size:"56 KB"},
                {id:4,name:"Cust_Q1.xlsx",template:"Customer Report",date:"Mar 19",rows:567,sheets:4,size:"410 KB"},
                {id:5,name:"CashFlow_Q1.xlsx",template:"Cash Flow Forecast",date:"Mar 17",rows:156,sheets:5,size:"320 KB"},
                {id:6,name:"MktgROI_Feb.xlsx",template:"Marketing ROI",date:"Mar 14",rows:234,sheets:4,size:"290 KB"},
              ].map((exp)=>(
                <div key={exp.id} style={{display:"grid",gridTemplateColumns:"2.2fr 1.4fr 0.9fr 0.7fr 0.7fr 0.8fr 0.6fr",padding:"13px 22px",borderBottom:"1px solid #F0F4EE",alignItems:"center",fontSize:12}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    <button style={{padding:"5px 7px",borderRadius:6,border:"1px solid #DDE8DA",background:"#F6F9F4",cursor:"pointer",color:"#43A047",display:"flex",alignItems:"center"}}><I.Download s={12}/></button>
                    <button style={{padding:"5px 7px",borderRadius:6,border:"1px solid #DDE8DA",background:"#F6F9F4",cursor:"pointer",color:"#93B593",display:"flex",alignItems:"center"}}><I.Refresh s={12}/></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:16}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                ...
                <button onClick={()=>{setIsLoggedIn(false);setAuthUser(null);toast("Signed out","Session terminated","info")}} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderRadius:10,border:"none",background:"transparent",color:"#D32F2F",fontSize:13,fontWeight:400,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><I.LogOut s={14}/>Log Out</button>
              </div>
                ...
                {settingsTab === "profile" && (
                  <div>
                    ...
                    <div style={{display:"flex",gap:20,alignItems:"flex-start",marginBottom:24}}>
                      <div style={{width:72,height:72,borderRadius:18,background:"linear-gradient(135deg,#66BB6A,#2E7D32)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:26,fontWeight:700,flexShrink:0}}>JD</div>
                      <div style={{flex:1}}>
                        <button style={{padding:"7px 16px",borderRadius:8,border:"1px solid #DDE8DA",background:"#F6F9F4",color:"#43A047",fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:"inherit",marginBottom:6}}>Change Avatar</button>
                        <div style={{fontSize:10,color:"#93B593"}}>JPG, PNG or GIF. Max 2MB.</div>
                      </div>
                    </div>
                    {[{label:"Full Name",value:"Jane Doe",type:"text"},{label:"Email",value:"jane@acmecorp.com",type:"email"},{label:"Company",value:"Acme Corp",type:"text"},{label:"Role",value:"Finance Manager",type:"text"},{label:"Timezone",value:"America/New_York (EST)",type:"text"}].map((f,i)=>(
                      <div key={i} style={{marginBottom:16}}>
                        ...
                        <input defaultValue={f.value} type={f.type} style={{width:"100%",padding:"10px 14px",borderRadius:8,border:"1px solid #DDE8DA",background:"#F6F9F4",fontSize:13,color:"#1a2e1a",fontFamily:"inherit",outline:"none"}}/>
                      </div>
                    ))}
                    <button style={{padding:"10px 24px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#2E7D32,#43A047)",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 2px 8px rgba(46,125,50,0.25)"}} onClick={()=>toast("Saved","Profile updated","success")}>Save Changes</button>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div style={{padding:"18px 20px",borderRadius:12,border:"2px solid #FFCDD2",background:"#FFF5F5",marginBottom:14}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                        ...
                        <button style={{padding:"8px 16px",borderRadius:8,border:"1px solid #FFCDD2",background:"#fff",color:"#D32F2F",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Export Data</button>
                      </div>
                    </div>
        ...
        {tab === "help" && (
          <div style={{ animation: "fadeUp 0.35s ease" }}>
            ...
            {/* Contact Form */}
            <div style={{background:"#fff",borderRadius:14,padding:28,border:"1px solid #E4EDE2",marginTop:16}}>
        ...
        <div style={{maxWidth:1220,margin:"0 auto",padding:"0 20px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr 1fr",gap:32,marginBottom:24}}>
            <div>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <div style={{background:"#E8F5E9",borderRadius:8,width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",color:"#2E7D32"}}><I.Bolt s={16}/></div>
            ...
            <div>
              ...
              {["Terms of Service","Privacy Policy","Cookie Policy","GDPR","SOC 2 Compliance"].map((l)=><div key={l} style={{fontSize:12,color:"#6B8F6B",marginBottom:6,cursor:"pointer"}}>{l}</div>)}
            </div>
          </div>
          <div style={{borderTop:"1px solid #F0F4EE",paddingTop:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:16}}>
              {["Status Page","API Docs","Security"].map((l)=><span key={l} style={{fontSize:11,color:"#6B8F6B",cursor:"pointer"}}>{l}</span>)}
            </div>
          </div>
        </div>
      </footer>
      ...
      {/* ═══ AUTH SCREEN ═══ */}
      {!isLoggedIn && (
        <div style={{position:"fixed",inset:0,background:"linear-gradient(135deg,#F4F8F2 0%,#E8F5E9 50%,#C8E6C9 100%)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{width:440,maxWidth:"90vw",animation:"fadeUp 0.4s ease"}}>
            <div style={{textAlign:"center",marginBottom:28}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:10,marginBottom:12}}>
                <div style={{background:"linear-gradient(135deg,#2E7D32,#43A047)",borderRadius:14,width:48,height:48,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff"}}><I.Bolt s={26}/></div>
                <span style={{fontFamily:"'IBM Plex Mono',monospace",fontWeight:700,fontSize:26,color:"#1B5E20"}}>ExcelBolt</span>
              </div>
              <p style={{fontSize:14,color:"#6B8F6B",margin:0}}>Turn raw data into Excel sheets businesses love.</p>
            </div>
            <div style={{background:"#fff",borderRadius:20,padding:32,boxShadow:"0 12px 48px rgba(27,94,32,0.12)",border:"1px solid #E4EDE2"}}>
              <div style={{display:"flex",marginBottom:24,background:"#F6F9F4",borderRadius:10,padding:3}}>
                {["login","signup"].map((m)=>(
                  <button key={m} onClick={()=>{setAuthMode(m);setAuthErrors({})}} style={{flex:1,padding:"9px 0",borderRadius:8,border:"none",background:authMode===m?"#2E7D32":"transparent",color:authMode===m?"#fff":"#6B8F6B",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s"}}>{m==="login"?"Log In":"Sign Up"}</button>
                ))}
              </div>
              <div>
                {authMode === "signup" && (
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
                    <div><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.firstName?"#D32F2F":"#6B8F6B",marginBottom:4}}>First Name {authErrors.firstName&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.firstName}</span>}</label><input onChange={(e)=>authRefs.current.firstName=e.target.value} placeholder="Jane" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.firstName?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                    <div><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.lastName?"#D32F2F":"#6B8F6B",marginBottom:4}}>Last Name {authErrors.lastName&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.lastName}</span>}</label><input onChange={(e)=>authRefs.current.lastName=e.target.value} placeholder="Doe" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.lastName?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                  </div>
                )}
                <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.email?"#D32F2F":"#6B8F6B",marginBottom:4}}>Email {authErrors.email&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.email}</span>}</label><input onChange={(e)=>authRefs.current.email=e.target.value} type="email" placeholder="jane@acmecorp.com" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.email?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                <div style={{marginBottom:authMode==="login"?8:14}}><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.password?"#D32F2F":"#6B8F6B",marginBottom:4}}>Password {authErrors.password&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.password}</span>}</label><input onChange={(e)=>authRefs.current.password=e.target.value} type="password" placeholder="••••••••••" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.password?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                {authMode === "signup" && (
                  <>
                    <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.confirmPassword?"#D32F2F":"#6B8F6B",marginBottom:4}}>Confirm Password {authErrors.confirmPassword&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.confirmPassword}</span>}</label><input onChange={(e)=>authRefs.current.confirmPassword=e.target.value} type="password" placeholder="••••••••••" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.confirmPassword?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                    <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,fontWeight:600,color:authErrors.company?"#D32F2F":"#6B8F6B",marginBottom:4}}>Company {authErrors.company&&<span style={{color:"#D32F2F",fontWeight:400}}>· {authErrors.company}</span>}</label><input onChange={(e)=>authRefs.current.company=e.target.value} placeholder="Acme Corp" style={{width:"100%",padding:"10px 14px",borderRadius:8,border:`1px solid ${authErrors.company?"#FFCDD2":"#DDE8DA"}`,background:"#F6F9F4",fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
                  </>
                )}
                {authMode==="login" && <div style={{textAlign:"right",marginBottom:16}}><button style={{background:"none",border:"none",color:"#43A047",fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:500}}>Forgot password?</button></div>}
                <button disabled={authLoading} onClick={()=>{
                  const fields = authRefs.current;
                  const errs = validateAuth(authMode, fields);
                  setAuthErrors(errs);
                  if (Object.keys(errs).length > 0) return;
                  setAuthLoading(true);
                  setTimeout(()=>{
                    const name = authMode==="signup" ? `${fields.firstName} ${fields.lastName}` : "Jane Doe";
                    const email = fields.email;
                    const jwt = generateJWT(email, name);
                    const user = { email, name, company: fields.company||"Acme Corp", jwt, expiresAt: Date.now()+86400000, sessionId: `sess_${Date.now().toString(36)}` };
                    setAuthUser(user);
                    setIsLoggedIn(true);
                    setAuthLoading(false);
                    setAuthErrors({});
                    if (authMode==="signup") setShowOnboarding(true);
                    toast(authMode==="login"?"Welcome back":"Account created", authMode==="login"?`Authenticated as ${name} · JWT issued`:`30-day trial started · Session ${user.sessionId}`, "success");
                  }, 1200);
                }} style={{width:"100%",padding:"12px 0",borderRadius:10,border:"none",background:authLoading?"#93B593":"linear-gradient(135deg,#1B5E20,#2E7D32,#43A047)",color:"#fff",fontSize:14,fontWeight:700,cursor:authLoading?"wait":"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px rgba(27,94,32,0.3)",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              ...
              {authMode==="signup" && <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><input type="checkbox" defaultChecked style={{accentColor:"#2E7D32"}}/><span style={{fontSize:11,color:"#6B8F6B"}}>I agree to the <span style={{color:"#43A047",cursor:"pointer"}}>Terms of Service</span> and <span style={{color:"#43A047",cursor:"pointer"}}>Privacy Policy</span></span></div>}
              <div style={{textAlign:"center",fontSize:12,color:"#93B593"}}>
                ...
                {["Google","Microsoft"].map((p)=>(
                  <button key={p} onClick={()=>{
                    setAuthLoading(true);
                    setTimeout(()=>{
                      const jwt=generateJWT(`jane@${p.toLowerCase()}.com`,"Jane Doe");
                      setAuthUser({email:`jane@${p.toLowerCase()}.com`,name:"Jane Doe",company:"Acme Corp",jwt,expiresAt:Date.now()+86400000,sessionId:`sess_${Date.now().toString(36)}`});
                      setIsLoggedIn(true);setAuthLoading(false);
                      toast("Welcome",`SSO via ${p} · JWT issued`,"success");
                    },800);
                  }} style={{width:"100%",padding:"10px 0",borderRadius:8,border:"1px solid #DDE8DA",background:"#F6F9F4",color:"#3D5C3D",fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"inherit",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>Continue with {p}</button>
                ))}
              </div>
              {authUser && <div style={{marginTop:12,padding:"8px 12px",background:"#F6F9F4",borderRadius:8,fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#93B593",wordBreak:"break-all"}}>Last session: {authUser.sessionId} · JWT: {authUser.jwt?.slice(0,40)}...</div>}
            </div>
          </div>
        </div>
      )}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:theme.card,borderRadius:16,width:520,maxWidth:"90vw",padding:28,boxShadow:dm?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(10,30,10,0.3)",border:`1px solid ${theme.border}`,animation:"fadeUp 0.15s ease"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                ...
                {k:"⌘ 1-7",d:"Switch tabs"},{k:"?",d:"Show shortcuts"},
                {k:"⌘ E",d:"New export"},{k:"⌘ ⇧ S",d:"Sync all sources"},
                {k:"⌘ B",d:"Batch export mode"},{k:"ESC",d:"Close modals"},
              ].map((s,i)=>(
      ...
      {isLoggedIn && (
        <div style={{position:"fixed",bottom:0,left:0,right:0,height:28,background:dm?"#0D1117":"#fff",borderTop:`1px solid ${theme.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px",zIndex:90,fontSize:10,color:theme.muted,fontFamily:"'IBM Plex Mono',monospace"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <span>Session: {authUser?.sessionId||"—"}</span>
